...
**Both** the TUI and a new CLI batch tool must set / clear `Alert` from the same rule whenever WHOIS or DNS results are updated (background scan, **`u`**, domain-added, and the CLI pass). The CLI additionally **prints** every domain where `alert` is true after its run.
...
```text
Alert = (expired OR soon OR dns-empty)
```
...
- Hard WHOIS or DNS **errors** do not set alert by themselves. Log ERROR and continue; do not abort the run. Prior `Alert` is **recomputed** from whatever expiry/DNS state is available after this pass (error on DNS → treat as not dns-empty for this classification).
- **Always set** `Alert` to `true` or `false` after classification — clear it when no reasons apply so a domain that recovered (renewed, DNS filled) drops off the watchlist on the next check (TUI or CLI).
...
### 1. TUI: refresh `Alert` after WHOIS or DNS updates

Whenever a domain’s WHOIS and/or DNS state is updated in the TUI, recompute `Alert` from the **current** `Expiry` + `DNS` (and clear it when the sniping signals are gone).

| Path | When to call `RefreshAlert` |
|------|-----------------------------|
| Domains panel **`u`** (`checkDomain`) | After both forced WHOIS and DNS finish for that domain (same action already runs both) |
| Background WHOIS (`whoisRunner` / `refresh`) | After each successful or attempted WHOIS update on a domain (recompute using new expiry + existing DNS) |
| Background DNS (`dnsRunner` / `dnsrefresh`) | After each DNS update (recompute using existing expiry + new DNS) |
| Domain-added jobs (`NewWhoisOnAdd` / `NewDnsOnAdd`) | After each job’s update on that domain |
...
- WHOIS and DNS runners stay independent ([SPEC_DNS_EMPTY.md](SPEC_DNS_EMPTY.md)): after a WHOIS-only pass, alert may flip from expiry alone; after a DNS-only pass, from emptiness alone; either path must not require the other runner to finish first.
...
- Lookup **errors** still trigger recompute (so a prior true alert can clear if the remaining good data no longer qualifies — or stay true if expiry/DNS still qualify). Errors alone never force `alert` true.
...
| `-delay` | `1s` | Sleep between domain check pairs (WHOIS+DNS), matching background refresh politeness |
| `-dry-run` | `false` | Load JSON, classify from **persisted** `Expiry` / `dns`, compute what `alert` *would* be; **no** network; **no** save; still print would-be alerts |
| `-force` | live always force | Live mode always checks every domain (ignore 10-day throttle) |
| `-save` | `true` | After live run, write WHOIS/DNS/`alert` via `db.WriteMps` once at end if anything changed. `-save=false` prints report but leaves disk unchanged |
...
3. For each MP → each domain (MP order, then domain order):
   ...
   b. domain.RefreshAlert(now, soonWindow)   // shared with TUI
   c. If domain.Alert: append to report rows (include reasons for humans).
4. If live && -save && any mutation (whois/dns/alert): db.WriteMps once.
5. Print report to stdout (alert == true only).
6. Exit 0 always in v1.
```

Reuse existing packages — do **not** reimplement WHOIS/DNS:

- `db` load/save
- `data.Domain.UpdateExpiryInfo` / `UpdateDns`
...
```text
example.com.au	mp=Jane Doe	alert=true	expiry=2026-10-01	reasons=soon,dns-empty	dns=empty
expired.example	mp=John Smith	alert=true	expiry=2025-01-01	reasons=expired	dns=ok
nx.example.au	mp=Alex Lee	alert=true	expiry=2027-06-01	reasons=dns-empty	dns=nxdomain
```
...
```text
# checked=42 alert=3 expired=1 soon=2 dns-empty=2 errors=1
```
...
- [ ] Shared `RefreshAlert` / `ComputeAlert` sets `Alert` true iff expired, soon (default 90d), or dns-empty; false otherwise
...
2. Wire `RefreshAlert` into TUI paths: **`u`**, whois refresh, dns refresh, domain-added jobs.
